...
class SparrowParsePipeline(Pipeline):
    ...
    def run_pipeline(
        ...
        if query == "*":
            ...
            try:
                query, query_schema = self.invoke_pipeline_step(
                    lambda: self.prepare_query_and_schema(query, debug),
        ...
        llm_output = self.invoke_pipeline_step(
            lambda: self.execute_query(
                options, query_all_data, query, file_path, debug
            ),
            "Executing query",
            local,
        )
        ...
        if query_all_data is False:
            validation_result = self.invoke_pipeline_step(
                lambda: self.validate_result(
                    llm_output, query_all_data, query_schema, debug
                ),
                "Validating result",
                local,
            )
    ...
    def execute_query(self, options, query_all_data, query, file_path, debug):
        ...
        input_data = [{"image": file_path, "text_input": query}]

        # Now you can run inference without knowing which implementation is used
        llm_output = extractor.run_inference(
